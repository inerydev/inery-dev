...
int main(int, char*[]) {
    ...
    OutputStream inery(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(inery);     // CHANGED
#else
    ...
    OutputStream inery(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(inery);             // CHANGED
#endif
