...
from compile_inery_tests import CompileError

import compile_inery_tests
import generate_inery_tests
...
def generate_wasm_and_copy():
    ...
    for d in os.listdir():
        ...
        if wasm_file in altered_wasms:
            ...
            try:
                generate_inery_tests.main(g_wasm_file, t_wasm_file, o_wast_file, map_file)
                wasm_file_path = os.path.join(d, wasm_file)
                out = subprocess.run(
                    ['inery-wast2wasm', o_wast_file, '-o', os.path.join(d, wasm_file)],
                    capture_output=True
                )
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("""Please provide:
                ...
                ex:
                python setup_inery_tests.py ~/code/inery-vm-test-wasms ~/code/inery ~/wasm_spec_tests
              """)
        sys.exit(1)
    ...
    generator = os.path.join(GENERATOR_DIR, 'inery_test_generator')
