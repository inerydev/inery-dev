...
#include <inery/testing/chainbase_fixture.hpp>

#include <boost/test/unit_test.hpp>

using namespace inery::chain::resource_limits;
...
BOOST_AUTO_TEST_SUITE(resource_limits_test)
   ...
   BOOST_FIXTURE_TEST_CASE(elastic_cpu_relax_contract, resource_limits_fixture) try {
      ...
      const uint64_t expected_contract_iterations =
              expected_exponential_average_iterations(0, INE_PERCENT(config::default_max_block_cpu_usage, config::default_target_block_cpu_usage_pct), config::default_max_block_cpu_usage, config::block_cpu_usage_average_window_ms / config::block_interval_ms ) +
   ...
   BOOST_FIXTURE_TEST_CASE(elastic_net_relax_contract, resource_limits_fixture) try {
      ...
      // subtracting 1 for the iteration that pulls double duty as reaching/exceeding the target and starting congestion handling
      const uint64_t expected_contract_iterations =
              expected_exponential_average_iterations(0, INE_PERCENT(config::default_max_block_net_usage, config::default_target_block_net_usage_pct), config::default_max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms ) +
...
// TODO: restore weighted capacity cpu tests
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_limit, resource_limits_fixture) try {
      ...
      for (uint64_t idx = 0; idx < expected_iterations - 1; idx++) {
         add_pending_mem_usage(account, increment);
         verify_account_mem_usage(account);
      }

      add_pending_mem_usage(account, increment);
      BOOST_REQUIRE_THROW(verify_account_mem_usage(account), mem_usage_exceeded);
   } FC_LOG_AND_RETHROW();
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_limit_underflow, resource_limits_fixture) try {
      ...
      verify_account_mem_usage(account);
      process_account_limit_updates();
      BOOST_REQUIRE_THROW(add_pending_mem_usage(account, -101), transaction_exception);
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_limit_overflow, resource_limits_fixture) try {
      ...
      verify_account_mem_usage(account);
      process_account_limit_updates();
      add_pending_mem_usage(account, UINT64_MAX/2);
      verify_account_mem_usage(account);
      add_pending_mem_usage(account, UINT64_MAX/2);
      verify_account_mem_usage(account);
      BOOST_REQUIRE_THROW(add_pending_mem_usage(account, 2), transaction_exception);
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_commitment, resource_limits_fixture) try {
      ...
      set_account_limits(account, limit, -1, -1 );
      process_account_limit_updates();
      add_pending_mem_usage(account, commit);
      verify_account_mem_usage(account);
