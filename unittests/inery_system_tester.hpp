...
namespace inery_system {

class inery_system_tester : public TESTER {
public:

   inery_system_tester()
   : inery_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   inery_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(inery.token), N(inery.ram), N(inery.ramfee), N(inery.stake),
               N(inery.bpay), N(inery.vpay), N(inery.saving), N(inery.names) });
      ...
      create_currency( N(inery.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("inery") ) );

      set_code( config::system_account_name, contracts::inery_system_wasm() );
      set_abi( config::system_account_name, contracts::inery_system_abi().data() );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("inery")) + get_balance(name("inery.ramfee")) + get_balance(name("inery.stake")) + get_balance(name("inery.ram")) );
   }
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, uint32_t mem_bytes = 8000 ) {
      ...
      trx.actions.emplace_back( get_action( config::system_account_name, N(buymembytes), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("bytes", mem_bytes) )
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( config::system_account_name, N(buyresources), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
   ...
   action_result buyresources( const account_name& payer, account_name receiver, const asset& inein ) {
      return push_action( payer, N(buyresources), mvo()( "payer",payer)("receiver",receiver)("quant",inein) );
   }
   action_result buymembytes( const account_name& payer, account_name receiver, uint32_t numbytes ) {
      return push_action( payer, N(buymembytes), mvo()( "payer",payer)("receiver",receiver)("bytes",numbytes) );
   }

   action_result sellres( const account_name& account, uint64_t numbytes ) {
      return push_action( account, N(sellres), mvo()( "account", account)("bytes",numbytes) );
   }
   ...
   action_result vote( const account_name& voter, const std::vector<account_name>& producers, const account_name& proxy = name(0) ) {
      return push_action(voter, N(makeprod), mvo()
                         ("voter",     voter)
                         ("proxy",     proxy)
                         ("producers", producers));
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(inery.token), act, N(accounts), name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(inery.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(inery.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "inery_global_state", data, abi_serializer::create_yield_function( abi_serializer_max_time ) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(inery.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyresources( name("inery"), name("inery.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(inery.msig), contracts::inery_msig_wasm() );
         set_abi( N(inery.msig), contracts::inery_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(inery.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total INE supply to activate chain
      transfer( name("inery"), name("alice1111111"), core_from_string("650000000.0000"), name("inery") );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::inery_code_name}, 1},
      ...
      //vote for producers
      {
         ...
         BOOST_REQUIRE_EQUAL(success(), buyresources( name("alice1111111"), name("alice1111111"), core_from_string("30000000.0000") ) );
         BOOST_REQUIRE_EQUAL(success(), push_action(N(alice1111111), N(makeprod), mvo()
                                                    ("voter",  "alice1111111")
   ...
   void cross_15_percent_threshold() {
      ...
      regproducer(N(producer1111));
      {
         ...
         trx.actions.emplace_back( get_action( config::system_account_name, N(makeprod),
...
inline uint64_t M( const string& ine_str ) {
   return core_from_string( ine_str ).get_amount();
}
