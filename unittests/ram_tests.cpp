...
#include "inery_system_tester.hpp"
 ...
 * ram_tests test case
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(ram_tests, inery_system::inery_system_tester) { try {
   ...
   buymembytes(config::system_account_name, config::system_account_name, 70000);
   produce_blocks(10);
   ...
   produce_blocks(10);
   BOOST_REQUIRE_EQUAL( success(), stake( name("inery.stake"), name("testram11111"), core_from_string("10.0000"), core_from_string("5.0000") ) );
   produce_blocks(10);

   for (auto i = 0; i < 10; ++i) {
      try {
         set_code( N(testram11111), contracts::test_mem_limit_wasm() );
         break;
      } catch (const mem_usage_exceeded&) {
         init_request_bytes += increment_contract_bytes;
         buymembytes(config::system_account_name, N(testram11111), increment_contract_bytes);
         buymembytes(config::system_account_name, N(testram22222), increment_contract_bytes);
      }
   }
   produce_blocks(10);
   ...
   for (auto i = 0; i < 10; ++i) {
      try {
         set_abi( N(testram11111), contracts::test_mem_limit_abi().data() );
         break;
      } catch (const mem_usage_exceeded&) {
         init_request_bytes += increment_contract_bytes;
         buymembytes(config::system_account_name, N(testram11111), increment_contract_bytes);
   ...
   produce_blocks(10);
   set_code( N(testram22222), contracts::test_mem_limit_wasm() );
   set_abi( N(testram22222), contracts::test_mem_limit_abi().data() );
   produce_blocks(10);

   auto total = get_total_stake( N(testram11111) );
   const auto init_bytes =  total["mem_bytes"].as_uint64();

   auto rlm = control->get_resource_limits_manager();
   auto initial_mem_usage = rlm.get_account_mem_usage(N(testram11111));
   ...
   wdump((init_bytes)(initial_mem_usage)(init_request_bytes)(more_ram) );
   buymembytes(config::system_account_name, N(testram11111), more_ram);
   buymembytes(config::system_account_name, N(testram22222), more_ram);
   ...
   tester->push_action( N(testram11111), N(setentry), N(testram11111), mvo()
                        ...
                        ("size", 1780 /*1910*/));
   produce_blocks(1);
   auto mem_usage = rlm.get_account_mem_usage(N(testram11111));

   total = get_total_stake( N(testram11111) );
   const auto mem_bytes =  total["mem_bytes"].as_uint64();
   wdump((mem_bytes)(mem_usage)(initial_mem_usage)(init_bytes)(mem_usage - initial_mem_usage)(init_bytes - mem_usage) );
   ...
   wlog("ram_tests 2    %%%%%%");
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL(mem_usage, rlm.get_account_mem_usage(N(testram11111)));
   ...
   tester->push_action( N(testram11111), N(setentry), N(testram11111), mvo()
                        ...
                        ("size", 1680/*1810*/));
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL(mem_usage - 1000, rlm.get_account_mem_usage(N(testram11111)));
