...
BOOST_AUTO_TEST_CASE( activate_preactivate_feature ) try {
   ...
   BOOST_CHECK_EXCEPTION( c.set_code( config::system_account_name, contracts::inery_bios_wasm() ),
                          wasm_exception, fc_exception_message_is("env.is_feature_activated unresolveable")
   );

   // But the old bios contract can still be set.
   c.set_code( config::system_account_name, contracts::before_preactivate_inery_bios_wasm() );
   c.set_abi( config::system_account_name, contracts::before_preactivate_inery_bios_abi().data() );
   ...
   BOOST_CHECK_EXCEPTION( c.push_action( config::system_account_name, N(reqactivated), config::system_account_name,
                           ...
                           inery_assert_message_exception,
                           inery_assert_message_is( "protocol feature is not activated" )
   );
...
BOOST_AUTO_TEST_CASE( replace_deferred_test ) try {
   ...
   auto alice_mem_usage0 = c.control->get_resource_limits_manager().get_account_mem_usage( N(alice) );
   ...
   auto alice_mem_usage1 = c.control->get_resource_limits_manager().get_account_mem_usage( N(alice) );
   ...
   BOOST_CHECK_EQUAL( c.control->get_resource_limits_manager().get_account_mem_usage( N(alice) ), alice_mem_usage1 );
   ...
   BOOST_CHECK_EQUAL( c.control->get_resource_limits_manager().get_account_mem_usage( N(alice) ), alice_mem_usage0 );
   ...
   auto alice_mem_usage2 = c.control->get_resource_limits_manager().get_account_mem_usage( N(alice) );
   BOOST_CHECK_EQUAL( alice_mem_usage2, alice_mem_usage1 + (alice_mem_usage1 - alice_mem_usage0) );
   ...
   auto alice_mem_usage3 = c.control->get_resource_limits_manager().get_account_mem_usage( N(alice) );
   BOOST_CHECK_EQUAL( alice_mem_usage3, alice_mem_usage1 );
...
BOOST_AUTO_TEST_CASE( no_duplicate_deferred_id_test ) try {
   ...
   // Retire the delayed inery::reqauth transaction.
   c.produce_blocks(5);
   BOOST_REQUIRE( trace1 );
   BOOST_REQUIRE_EQUAL(0, index.size());
...
BOOST_AUTO_TEST_CASE( fix_linkauth_restriction ) { try {
   ...
   auto validate_disallow = [&] (const char *code, const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link inery::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };

   validate_disallow("inery", "linkauth");
   validate_disallow("inery", "unlinkauth");
   validate_disallow("inery", "deleteauth");
   validate_disallow("inery", "updateauth");
   validate_disallow("inery", "canceldelay");
...
BOOST_AUTO_TEST_CASE( forward_setcode_test ) { try {
   ...
   //   * inery::setcode to set code on the inery is allowed (unless the rejectall account exists)
   //   * inery::newaccount is allowed only if it creates the rejectall account.
   c.set_code( config::system_account_name, contracts::reject_all_wasm() );
   c.produce_block();

   // Before activation, deploying a contract should work since setcode won't be forwarded to the WASM on inery.
   c.set_code( tester1_account, contracts::noop_wasm() );

   // Activate FORWARD_SETCODE protocol feature and then return contract on inery back to what it was.
   const auto& pfm = c.control->get_protocol_feature_manager();
   ...
   // After activation, deploying a contract causes setcode to be dispatched to the WASM on inery,
   ...
   BOOST_REQUIRE_EXCEPTION( c.set_code( tester2_account, contracts::noop_wasm() ),
                            inery_assert_message_exception,
                            inery_assert_message_is( "rejecting all actions" ) );
   ...
   // It will now not be possible to deploy the reject_all contract to the inery account,
   ...
   // already deployed on inery.
   c2.create_account( N(rejectall) );
   c2.produce_block();
   // Now all actions dispatched to the inery account should be rejected.

   // However, it should still be possible to set the bios contract because the WASM on inery is called after the
...
BOOST_AUTO_TEST_CASE( get_sender_test ) { try {
   ...
   BOOST_CHECK_EXCEPTION(  c.push_action( tester1_account, N(sendinline), tester1_account, mutable_variant_object()
                           ...
                           inery_assert_message_exception,
                           inery_assert_message_is( "sender did not match" ) );
...
BOOST_AUTO_TEST_CASE( mem_restrictions_test ) { try {
   tester c( setup_policy::preactivate_feature_and_new_bios );
   ...
   c.produce_block();
   c.set_code( tester1_account, contracts::mem_restrictions_test_wasm() );
   c.set_abi( tester1_account, contracts::mem_restrictions_test_abi().data() );
   c.produce_block();
   c.set_code( tester2_account, contracts::mem_restrictions_test_wasm() );
   c.set_abi( tester2_account, contracts::mem_restrictions_test_abi().data() );
   c.produce_block();
   ...
   // Cannot bill more MEM to another account that has not authorized the action.
   BOOST_REQUIRE_EXCEPTION(
   ...
   // in a MEM usage neutral way without the authority of that account.
   BOOST_REQUIRE_EXCEPTION(
      c.push_action( tester1_account, N(setdata), bob_account, mutable_variant_object()
         ("len1", 0)
         ("len2", 10)
   ...
   // Cannot bill more MEM to another account within a notification
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      subjective_block_production_exception,
      fc_exception_message_is( "Cannot charge MEM to other accounts during notify." )
   );

   // Cannot migrate data from table1 to table2 paid by another account
   // in a MEM usage neutral way within a notification.
   // This is due to the subjective mitigation in place.
   BOOST_REQUIRE_EXCEPTION(
      c.push_action( tester2_account, N(notifysetdat), alice_account, mutable_variant_object()
         ("acctonotify", tester1_account)
   ...
   // in a MEM usage neutral way with the authority of that account.
   c.push_action( tester1_account, N(setdata), alice_account, mutable_variant_object()
      ("len1", 0)
      ("len2", 10)
      ("payer", alice_account)
   ...
   // Without the subjective mitigation, it is now possible to bill more MEM to another account
   ...
   // Still cannot bill more MEM to another account within a notification
   // even if the account authorized the original action.
   // This is due to the subjective mitigation in place.
   BOOST_REQUIRE_EXCEPTION(
      c.push_action( tester2_account, N(notifysetdat), alice_account, mutable_variant_object()
   ...
   const auto& pfm = c.control->get_protocol_feature_manager();
   const auto& d = pfm.get_builtin_digest( builtin_protocol_feature_t::mem_restrictions );
   BOOST_REQUIRE( d );

   // Activate MEM_RESTRICTIONS protocol feature (this would also disable the subjective mitigation).
   c.preactivate_protocol_features( {*d} );
   c.produce_block();
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      action_validate_exception,
      fc_exception_message_starts_with( "cannot bill MEM usage of deferred transaction to another account that has not authorized the action" )
   );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      action_validate_exception,
      fc_exception_message_is( "cannot bill MEM usage of deferred transactions to another account within notify context" )
   );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      unauthorized_mem_usage_increase,
      fc_exception_message_starts_with( "unprivileged contract cannot increase MEM usage of another account within a notify context" )
   );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      unauthorized_mem_usage_increase,
      fc_exception_message_starts_with( "unprivileged contract cannot increase MEM usage of another account that has not authorized the action" )
   );
   ...
   // in a MEM usage neutral way even within a notification .
   ...
   // in a way that reduces MEM usage as well, even within a notification.
   c.push_action( tester2_account, N(notifysetdat), bob_account, mutable_variant_object()
      ("acctonotify", "tester1")
      ("len1", 0)
      ("len2", 5)
   ...
   // It should also still be possible for the receiver to take over payment of the MEM
   // if it is necessary to increase MEM usage without the authorization of the original payer.
...
BOOST_AUTO_TEST_CASE( webauthn_producer ) { try {
   ...
   BOOST_CHECK_THROW(
      ...
      inery::chain::unactivated_key_type
   );
...
BOOST_AUTO_TEST_CASE( webauthn_create_account ) { try {
   ...
   BOOST_CHECK_THROW(c.push_transaction(trx), inery::chain::unactivated_key_type);
...
BOOST_AUTO_TEST_CASE( webauthn_recover_key ) { try {
   ...
   BOOST_CHECK_THROW(c.push_transaction(trx), inery::chain::unactivated_signature_type);
...
BOOST_AUTO_TEST_CASE( producer_schedule_change_extension_test ) { try {
   ...
   { // ensure producer_schedule_change_extension is rejected
      ...
      bad_block->producer_signature = remote.get_private_key(N(inery), "active").sign(sig_digest);
...
BOOST_AUTO_TEST_CASE( wtmsig_block_signing_inflight_legacy_test ) { try {
   ...
   c.preactivate_protocol_features( {*d} );
   vector<legacy::producer_key> sched = {{N(inery), c.get_public_key(N(inery), "bsk")}};
   ...
   c.block_signing_private_keys.emplace(get_public_key(N(inery), "bsk"), get_private_key(N(inery), "bsk"));
   c.produce_block();
