...
public:
   ...
   fc::variant get_global_state() {
    ...
    auto buyresources( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(config::system_account_name, N(buyresources), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(inery.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create inery.msig and inery.token
        create_accounts({N(inery.msig), N(inery.token), N(inery.ram), N(inery.ramfee), N(inery.stake), N(inery.vpay), N(inery.bpay), N(inery.saving) });
        // Set code for the following accounts:
        //  - inery (code: inery.bios) (already set by tester constructor)
        //  - inery.msig (code: inery.msig)
        //  - inery.token (code: inery.token)
        // set_code_abi(N(inery.msig), contracts::inery_msig_wasm(), contracts::inery_msig_abi().data());//, &inery_active_pk);
        // set_code_abi(N(inery.token), contracts::inery_token_wasm(), contracts::inery_token_abi().data()); //, &inery_active_pk);

        set_code_abi(N(inery.msig),
                     contracts::inery_msig_wasm(),
                     contracts::inery_msig_abi().data());//, &inery_active_pk);
        set_code_abi(N(inery.token),
                     contracts::inery_token_wasm(),
                     contracts::inery_token_abi().data()); //, &inery_active_pk);

        // Set privileged for inery.msig and inery.token
        set_privileged(N(inery.msig));
        set_privileged(N(inery.token));

        // Verify inery.msig and inery.token is privileged
        const auto& inery_msig_acc = get<account_metadata_object, by_name>(N(inery.msig));
        BOOST_TEST(inery_msig_acc.is_privileged() == true);
        const auto& inery_token_acc = get<account_metadata_object, by_name>(N(inery.token));
        BOOST_TEST(inery_token_acc.is_privileged() == true);


        // Create SYS tokens in inery.token, set its manager as inery
        ...
        create_currency(N(inery.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to inery.system
        issue(N(inery.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyresources(config::system_account_name, a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(inery.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(config::system_account_name, N(makeprod), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("inery"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), inery_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), inery_assert_message_exception);
