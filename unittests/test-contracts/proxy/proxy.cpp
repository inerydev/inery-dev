...
proxy::proxy( inery::name self, inery::name first_receiver, inery::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      inery::token::transfer_action a( "inery.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, inery::ignore<std::vector<char>> ) {
