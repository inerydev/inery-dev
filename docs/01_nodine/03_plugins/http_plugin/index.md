...
The `http_plugin` is a core plugin supported by both `nodine` and `kined`. The plugin is required to enable any RPC API functionality provided by a `nodine` or `kined` instance.
...
```sh
# command-line
nodine ... --plugin inery::http_plugin [options]
 (or)
kined ... --plugin inery::http_plugin [options]
```
...
Config Options for inery::http_plugin:
  --unix-socket-path arg                The filename (relative to data-dir) to 
                                        ...
                                        blank to disable (=kined.sock for kined)
  --http-server-address arg (=127.0.0.1:8888 for nodine)
