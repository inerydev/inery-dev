...
```sh
# command-line
nodine ... --plugin inery::trace_api_plugin [options]
```
...
```console
Config Options for inery::trace_api_plugin:
...
Here is a `nodine` configuration example for the `trace_api_plugin` when tracing some INERY reference contracts:

```sh
nodine --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin inery::trace_api_plugin 
--trace-rpc-abi=inery=abis/inery.abi 
--trace-rpc-abi=inery.token=abis/inery.token.abi 
--trace-rpc-abi=inery.msig=abis/inery.msig.abi 
--trace-rpc-abi=inery.wrap=abis/inery.wrap.abi
```
