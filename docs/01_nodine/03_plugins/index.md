---
content_title: Nodine Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodine`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodine`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | Nodine is modular]]
| Plugins add incremental functionality to `nodine`. Unlike runtime plugins, `nodine` plugins are built at compile-time.
