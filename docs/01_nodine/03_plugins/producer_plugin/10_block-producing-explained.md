...
peer validation for similar hardware/inery-version/config will be <= m
...
#       * image will be rendered by gatsby-remark-graphviz plugin in inery docs.
...
Running wasm-runtime=ine-vm-jit ine-vm-oc-enable on relay node will reduce the validation time.
