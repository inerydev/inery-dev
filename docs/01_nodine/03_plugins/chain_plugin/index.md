...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an INERY node.
...
```console
Config Options for inery::chain_plugin:
  ...
  --wasm-runtime ine-vm|ine-vm-jit      Override default WASM runtime (wabt)
  --ine-vm-oc-enable                    Enable optimized compilation in WASM
  ...
  --disable-ram-billing-notify-checks   Disable the check which subjectively 
                                        ...
                                        more MEM to another account within the 
