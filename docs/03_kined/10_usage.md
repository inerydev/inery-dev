---
content_title: Kined Usage
---

[[info | Recommended Usage]]
| For most users, the easiest way to use `kined` is to have `cline` launch it automatically. Wallet files will be created in the default directory (`~/inery-wallet`).

## Launching kined manually

`kined` can be launched manually from the terminal by running:

```sh
kined
```

By default, `kined` creates the folder `~/inery-wallet` and populates it with a basic `config.ini` file.  The location of the config file can be specified on the command line using the `--config-dir` argument.  The configuration file contains the HTTP server endpoint for incoming HTTP connections and other parameters for cross-origin resource sharing.
...
By default, `kined` is set to lock your wallet after 15 minutes of inactivity. This is configurable in the `config.ini` by setting the timeout seconds in `unlock-timeout`. Setting it to 0 will cause `kined` to always lock your wallet.

## Stopping kined

The most effective way to stop `kined` is to find the kined process and send a SIGTERM signal to it.

## Other options

For a list of all commands known to `kined`, simply run it with no arguments:
...
Config Options for inery::http_plugin:
  --unix-socket-path arg (=kined.sock)  The filename (relative to data-dir) to
...
Config Options for inery::wallet_plugin:
