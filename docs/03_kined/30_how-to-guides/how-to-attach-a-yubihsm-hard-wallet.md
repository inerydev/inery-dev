...
* Install the currently supported version of `kined`
...
### Configure `kined`
   ...
   There are two options to connect `kined` to YubiHSM:

   #### Using a YubiHSM connector

   By default, `kined` will connect to the YubiHSM connector on the default host and port. If a non-default URL is used, set the `--yubihsm-url` option or `yubihsm-url` in `config.ini` with the correct connector URL

   #### Directly connect via USB

   `kined` also can directly connect to YubiHSM via USB protocol

   If this option is used, set `kined` startup option as the below:

   ```sh
   --yubihsm-url=ysb://
   ```

### Start `kined` with AuthKey:
...
### Unlock YubiHSM wallet with the password of AuthKey using the following option:
   ...
   ```sh
   cline wallet unlock -n YubiHSM --password YOUR_AUTHKEY_PASSWORD
   ```

After unlocking the wallet, you can use `cline wallet` commands as usual. Beware as a part of security mechanism, some wallet subcommands, such as retrieve private keys, or remove a key, are not supported when a YubiHSM is used