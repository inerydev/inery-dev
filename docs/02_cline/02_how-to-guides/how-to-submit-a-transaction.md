...
[[info]]
| Be aware that if a clear text `data` field is used, cline need to fetch copies of required ABIs using `nodine` API. That operation has a performance overhead on `nodine`
...
```sh
cline push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
cline push transaction JSON
```
