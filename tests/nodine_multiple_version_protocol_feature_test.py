...
###############################################################
# nodine_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of nodine can work with newer versions of nodine.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, nodinePath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, nodinePath=nodinePath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraNodineArgs=" --plugin inery::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of nodine that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodinePath="programs/nodine/nodine")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodinePath="programs/nodine/nodine")
