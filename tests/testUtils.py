...
class Utils:
    ...
    IneClientPath="programs/cline/cline"
    MiscIneClientArgs="--no-auto-kined"

    IneWalletName="kined"
    IneWalletPath="programs/kined/"+ IneWalletName

    IneServerName="nodine"
    IneServerPath="programs/nodine/"+ IneServerName

    IneLauncherPath="programs/inery-launcher/inery-launcher"
    MongoPath="mongo"
    ShuttingDown=False
    CheckOutputDeque=deque(maxlen=10)

    IneBlockLogPath="programs/inery-blocklog/inery-blocklog"
    ...
    def getBlockLog(blockLogLocation, blockLogAction=BlockLogAction.return_blocks, outputFile=None, first=None, last=None, throwException=False, silentErrors=False, exitOnError=False):
        ...
        cmd="%s --blocks-dir %s --as-json-array %s%s%s%s" % (Utils.IneBlockLogPath, blockLogLocation, outputFileStr, firstStr, lastStr, blockLogActionStr)
