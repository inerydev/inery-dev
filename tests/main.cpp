...
//extern uint32_t INE_TESTING_GENESIS_TIMESTAMP;
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
   ...
   /*
   const char* genesis_timestamp_str = getenv("INE_TESTING_GENESIS_TIMESTAMP");
   if( genesis_timestamp_str != nullptr )
   {
      INE_TESTING_GENESIS_TIMESTAMP = std::stoul( genesis_timestamp_str );
   }
   std::cout << "INE_TESTING_GENESIS_TIMESTAMP is " << INE_TESTING_GENESIS_TIMESTAMP << std::endl;
   */
   return nullptr;
}
