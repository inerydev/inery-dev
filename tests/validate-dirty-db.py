...
# validate-dirty-db
#
# Test for validating the dirty db flag sticks repeated nodine restart attempts
#
###############################################################
...
def runNodineAndGetOutput(myTimeout=3):
    """Startup nodine, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodine process.")
    cmd="programs/nodine/nodine --config-dir etc/inery/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("Nodine process launched.")

    output={}
    try:
        if debug: Print("Setting nodine process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("Nodine process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: Nodine is running beyond the defined wait time. Hard killing nodine instance.")
...
try:
    ...
    Print("Restart nodine repeatedly to ensure dirty database flag sticks.")
    timeout=6

    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = runNodineAndGetOutput(timeout)
        assert(ret)
        assert(isinstance(ret, tuple))
        if not ret[0]:
            errorExit("Failed to startup nodine successfully on try number %d" % (i))
        assert(ret[1])
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killIneInstances, False, keepLogs, killAll, dumpErrorDetails)
