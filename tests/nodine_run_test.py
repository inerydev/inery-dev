...
# nodine_run_test
#
# General test that tests a wide range of general use actions around nodine and kined
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.ineryAccount)
    ...
    transId=node.createInitializeAccount(currencyAccount, cluster.defproduceraAccount, buyMEM=200000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.defproduceraAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.defproduceraAccount, buyMEM=200000, waitForTransBlock=True, exitOnError=True)
    ...
    contractDir="unittests/contracts/inery.token"
    wasmFile="inery.token.wasm"
    abiFile="inery.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getIneAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get INE account failed", raw=True)
    ...
    account=node.getIneAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getIneAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
