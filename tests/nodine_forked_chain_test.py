...
try:
    ...
    specificExtraNodineArgs={}
    # producer nodes will be mapped to 0 through totalProducerNodes-1, so the number totalProducerNodes will be the non-producing node
    specificExtraNodineArgs[totalProducerNodes]="--plugin inery::test_control_api_plugin"
    ...
    if cluster.launch(prodCount=prodCount, topo="bridge", pnodes=totalProducerNodes,
                      ...
                      useBiosBootFile=False, specificExtraNodineArgs=specificExtraNodineArgs) is False:
        Utils.cmdError("launcher")
        Utils.errorExit("Failed to stand up ine cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.ineryAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.ineryAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyMEM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.ineryAccount.name, account.name))
        node.transferFunds(cluster.ineryAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
