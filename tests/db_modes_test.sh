...
INERY_STUFF_DIR=$(mktemp -d)
trap "rm -rf $INERY_STUFF_DIR" EXIT
NODINE_LAUNCH_PAMEMS="./programs/nodine/nodine -d $INERY_STUFF_DIR --config-dir $INERY_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -pinery"

run_nodine() {
   if (( $VERBOSE == 0 )); then
      $NODINE_LAUNCH_PAMEMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $NODINE_LAUNCH_PAMEMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $NODINE_PID; wait $NODINE_PID; exit 1" ALRM
   sleep 10
   if wait $NODINE_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
