...
###############################################################
# nodine_run_remote_test
#
#  Tests remote capability of the nodine_run_test. Test will setup cluster and pass nodes info to nodine_run_test. E.g.
#  nodine_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodine_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodine", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killIneInstances, False, False, killAll, dumpErrorDetails)
