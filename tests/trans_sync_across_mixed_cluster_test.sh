...
killAll()
{
  programs/inery-launcher/inery-launcher -k 15
}

cleanup()
{
    rm -rf etc/inery/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/cline/cline get info)"
  verifyErrorCode "cline get info"
...
# stand up nodine cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodine \"--plugin inery::wallet_api_plugin\" $launcherOpts
programs/inery-launcher/inery-launcher --nodine "--plugin inery::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cline/cline --port $port2 get block 1)"
    verifyErrorCode "cline get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cline/cline create key)"
verifyErrorCode "cline create key"
...
PASSWORD_INITA="$(programs/cline/cline wallet create --name inita)"
verifyErrorCode "cline wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cline/cline wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "cline wallet import"
programs/cline/cline wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "cline wallet import"
programs/cline/cline wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "cline wallet import"
programs/cline/cline wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "cline wallet import"
...
ACCOUNT_INFO="$(programs/cline/cline create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cline create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cline/cline get account testera)"
verifyErrorCode "cline get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cline/cline transfer inita testera 975321 "test transfer")"
    verifyErrorCode "cline transfer"
   ...
   TRANS_INFO="$(programs/cline/cline --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "cline get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
