...
###############################################################
# nodine_under_min_avail_ram
#
# Sets up 4 producing nodes using --chain-state-db-guard-size-mb and --chain-state-db-size-mb to verify that nodine will
...
killIneInstances=not dontKill
killWallet=not dontKill

WalletdName=Utils.IneWalletName
ClientName="cline"
...
try:
    ...
    minMEMFlag="--chain-state-db-guard-size-mb"
    minMEMValue=1002
    maxMEMFlag="--chain-state-db-size-mb"
    maxMEMValue=1010
    extraNodineArgs=" %s %d %s %d  --http-max-response-time-ms 990000 " % (minMEMFlag, minMEMValue, maxMEMFlag, maxMEMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodineArgs=extraNodineArgs, useBiosBootFile=False) is False:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.ineryAccount])
    ...
    # create accounts via inery as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.ineryAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.ineryAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyMEM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.ineryAccount.name, account.name))
        nodes[0].transferFunds(cluster.ineryAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.ineryAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.ineryAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyMEM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.ineryAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.ineryAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to inery contract for %d consecutive times, looks like nodine already exited." % (timeOutCount))
                        keepProcessing=False
                        break

                    Print("Failed to push create action to inery contract. sleep for 5 seconds")
    ...
    # min and max are subjective, just assigned to make sure that many small changes in nodine don't 
    ...
    addSwapFlags={}
    maxMEMValue+=2
    currentMinimumMaxMEM=maxMEMValue
    enabledStaleProduction=False
    for i in range(numNodes):
        addSwapFlags[maxMEMFlag]=str(maxMEMValue)
        ...
        if not nodes[nodeIndex].relaunch(nodeIndex, "", newChain=False, addSwapFlags=addSwapFlags):
            Utils.cmdError("Failed to restart node0 with new capacity %s" % (maxMEMValue))
            errorExit("Failure - Node should have restarted")
        addSwapFlags={}
        maxMEMValue=currentMinimumMaxMEM+30
    ...
    while keepProcessing and count < 40:
        ...
        for fromIndexOffset in range(namedAccounts.numAccounts):
            ...
            try:
                trans=nodes[count % numNodes].pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Print("Failed to push create action to inery contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
            except TypeError as ex:
    ...
    time.sleep(10)
    maxMEMValue=currentMinimumMaxMEM+5
    currentMinimumMaxMEM=maxMEMValue
    addSwapFlags[maxMEMFlag]=str(maxMEMValue)
    if not nodes[len(nodes)-1].relaunch(nodeIndex, "", newChain=False, addSwapFlags=addSwapFlags):
        Utils.cmdError("Failed to restart node %d with new capacity %s" % (numNodes-1, maxMEMValue))
        errorExit("Failure - Node should have restarted")
    addSwapFlags={}
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killIneInstances=killIneInstances, killWallet=killWallet, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
