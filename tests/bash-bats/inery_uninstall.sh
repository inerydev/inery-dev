...
SCRIPT_LOCATION="scripts/inery_uninstall.sh"
TEST_LABEL="[inery_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - INERY Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled INERY Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i inerytest
  [[ $output =~ .*/inerytest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/inery" ]] && [[ ! $output =~ ".local/share/inery" ]]) || exit
  [[ ! $output =~ "INERY Removal Complete" ]] || exit
}
