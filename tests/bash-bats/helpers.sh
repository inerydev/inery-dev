...
@test "${TEST_LABEL} > previous install prompt" {
  ...
  [[ -z $(echo "${output}") ]] || exit
  # Exists, prompt
  mkdir -p $INERY_INSTALL_DIR
  run previous-install-prompt
  [[ ! -z $(echo "${output}" | grep "INERY has already been installed into ${INERY_INSTALL_DIR}") ]] || exit
  rm -rf $INERY_INSTALL_DIR
}
