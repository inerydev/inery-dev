...
export SCRIPT_LOCATION="scripts/inery_build.sh"
export TEST_LABEL="[inery_build_darwin]"
...
## Needed to load inery_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting INERY Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap inery/inery") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting INERY Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "INERY has been successfully built") ]] || exit
}