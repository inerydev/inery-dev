...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="inery-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kined running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="INEtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kined running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: ine server host
        port: ine server port
        walletHost: ine wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures ine mongo plugin
        ...
        self.ineryAccount=self.defProducerAccounts["inery"]= Account("inery")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodineArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodineArgs=None, onlySetProds=False,
        ...
        extraNodineArgs: string of arguments to pass through to each nodoes instance (via --nodine flag on launcher)
        ...
        specificExtraNodineArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodine flags on launcher), example: { "5" : "--plugin inery::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the inery.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.IneServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodine default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.IneLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodineArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodineArgs += " --plugin inery::wallet_api_plugin"
        if self.enableMongo:
            nodineArgs += " --plugin inery::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodineArgs is not None:
            assert(isinstance(extraNodineArgs, str))
            nodineArgs += extraNodineArgs
        if Utils.Debug:
            nodineArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodineArgs += " --plugin inery::producer_api_plugin"

        if nodineArgs:
            cmdArr.append("--nodine")
            cmdArr.append(nodineArgs)

        if specificExtraNodineArgs is not None:
            assert(isinstance(specificExtraNodineArgs, dict))
            for nodeNum,arg in specificExtraNodineArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodine")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodine")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.IneServerName, startedNodes, len(nodes)))
            return False
        ...
        self.ineryAccount=self.defProducerAccounts["inery"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.IneClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.IneClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on ine server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextIneIdx: %d, n: %d" % (nextIneIdx, n))
                nextIneIdx=(nextIneIdx + 1)%count
                if not self.nodes[nextIneIdx].killed:
                    #Utils.Print("nextIneIdx: %d" % (nextIneIdx))
            ...
            #Utils.Print("nextIneIdx: %d, count: %d" % (nextIneIdx, count))
            node=self.nodes[nextIneIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.IneServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on ine node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getIneBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.ineryAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def createAccountAndVerify(self, account, creator, stakedDeposit=1000, stakeNet=100, stakeCPU=100, buyMEM=10000):
        """create account, verify account and return transaction id"""
        assert(len(self.nodes) > 0)
        node=self.nodes[0]
        trans=node.createInitializeAccount(account, creator, stakedDeposit, stakeNet=stakeNet, stakeCPU=stakeCPU, buyMEM=buyMEM, exitOnError=True)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyMEM=10000, exitOnError=False):
        assert(len(self.nodes) > 0)
        node=self.nodes[0]
        trans=node.createInitializeAccount(account, creatorAccount, stakedDeposit, waitForTransBlock, stakeNet=stakeNet, stakeCPU=stakeCPU, buyMEM=buyMEM)
        return trans
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse inery private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by inery-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/inery.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/inery.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down ine cluster.")
            return None
        ...
        ineryName="inery"
        ineryKeys=producerKeys[ineryName]
        ineryAccount=Account(ineryName)
        ineryAccount.ownerPrivateKey=ineryKeys["private"]
        ineryAccount.ownerPublicKey=ineryKeys["public"]
        ineryAccount.activePrivateKey=ineryKeys["private"]
        ineryAccount.activePublicKey=ineryKeys["public"]
        producerKeys.pop(ineryName)

        if not self.walletMgr.importKey(ineryAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (ineryName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="inery.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"inery\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init inery transfer")
            opts="--permission inery@active"
            if name != "inery":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from inery.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(ineryAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, ineryAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission inery@active"
                    myTrans=biosNode.pushMessage("inery", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("inery", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but inery).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "inery"
        ...
        ineryTokenAccount=copy.deepcopy(ineryAccount)
        ineryTokenAccount.name="inery.token"
        trans=biosNode.createAccount(ineryTokenAccount, ineryAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ineryTokenAccount.name))
            return None

        ineryRamAccount=copy.deepcopy(ineryAccount)
        ineryRamAccount.name="inery.ram"
        trans=biosNode.createAccount(ineryRamAccount, ineryAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ineryRamAccount.name))
            return None

        ineryRamfeeAccount=copy.deepcopy(ineryAccount)
        ineryRamfeeAccount.name="inery.ramfee"
        trans=biosNode.createAccount(ineryRamfeeAccount, ineryAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ineryRamfeeAccount.name))
            return None

        ineryStakeAccount=copy.deepcopy(ineryAccount)
        ineryStakeAccount.name="inery.stake"
        trans=biosNode.createAccount(ineryStakeAccount, ineryAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ineryStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(ineryTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=ineryTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (ineryAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to inery contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (ineryAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (ineryAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to inery contract.")
            return None
        ...
        Utils.Print("Verify inery issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountIneBalanceStr(ineryAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (ineryAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (ineryAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (ineryTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (ineryAccount.name)
            trans=biosNode.pushMessage(ineryAccount.name, action, data, opts)
    ...
    @staticmethod
    def pgrepIneServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.IneServerName)
    ...
    @staticmethod
    def pgrepIneServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of IneInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepIneServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepIneServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.IneServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepIneServers(timeout=timeout)
        pattern=Cluster.pgrepIneServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.IneServerName, pattern))
    ...
    # Kills a percentange of Ine instances starting from the tail and update ineInstanceInfos state
    def killSomeIneInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.IneServerName, killSignal))
    ...
    def relaunchIneInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodine instances. allInstances will kill all nodine instances running on the system."""
        cmd="%s -k 9" % (Utils.IneLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down ine cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the ine server
            cmd="pkill -9 %s" % (Utils.IneServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down ine cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodine instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodine instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
