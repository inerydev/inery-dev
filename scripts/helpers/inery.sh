...
function ensure-compiler() {
    ...
    if [[ $NAME == "CentOS Linux" ]] || [[ $VERSION_ID == "16.04" ]] || ( $PIN_COMPILER && [[ $VERSION_ID == "18.04" ]] || [[ "$(echo ${VERSION_ID})" == "20.04" ]]); then
        export CXX=${CXX:-'g++'}
        export CC=${CC:-'gcc'}
    fi
...
function ensure-llvm() {
	...
	execute bash -c "cp -r '${INR_PATH}'/clang_llvm_8/llvm '${LLVM_ROOT}'"
...
function build-clang() {
    if $BUILD_CLANG; then
	    ...
	    execute bash -c "cp -r '${INR_PATH}'/clang_llvm_8/clang8 '${CLANG_ROOT}'"
