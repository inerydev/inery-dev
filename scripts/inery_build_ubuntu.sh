...
( [[ $NAME == "Ubuntu" ]] && ( [[ "$(echo ${VERSION_ID})" == "16.04" ]] || [[ "$(echo ${VERSION_ID})" == "18.04" ]]  || [[ "$(echo ${VERSION_ID})" == "20.04" ]] ) ) || ( echo " - You must be running 16.04.x or 18.04.x to install INERY." && exit 1 )
...
( [[ $PIN_COMPILER == false ]] && [[ $VERSION_ID == "18.04" ]] || [[ "$(echo ${VERSION_ID})" == "20.04" ]]) && EXTRA_DEPS=(clang,dpkg\ -s llvm-7-dev,dpkg\ -s)
...
( [[ $VERSION_ID == "16.04" ]] || ( $PIN_COMPILER && [[ $VERSION_ID == "18.04" ]] || [[ "$(echo ${VERSION_ID})" == "20.04" ]]) ) && ensure-build-essential
