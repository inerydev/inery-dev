...
###############################################################
# nodine_chainbase_allocation_test
#
...
try:
    ...
    # (automatically taken care by the automatically generated inery account)
    assert cluster.launch(
        ...
        specificExtraNodineArgs={
            1:"--read-mode irreversible --plugin inery::producer_api_plugin"})
    ...
    cmd = "create account -j inery sample INE6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\
         INE6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV --delay-sec 600 -p inery"
    trans = producerNode.processClineCmd(cmd, cmd, silentErrors=False)
    assert trans

    # Schedule a new producer to trigger new producer schedule for "global_property_object"
    newProducerAcc = Account("newprod")
    newProducerAcc.ownerPublicKey = "INE6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    newProducerAcc.activePublicKey = "INE6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    producerNode.createAccount(newProducerAcc, cluster.ineryAccount)

    setProdsStr = '{"schedule": ['
    setProdsStr += '{"producer_name":' + newProducerAcc.name + ',"block_signing_key":' + newProducerAcc.activePublicKey + '}'
    setProdsStr += ']}'
    cmd="push action -j inery setprods '{}' -p inery".format(setProdsStr)
    trans = producerNode.processClineCmd(cmd, cmd, silentErrors=False)
    assert trans
