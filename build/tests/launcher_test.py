...
# launcher-test
#
# Specifically tests using the bios bootstrap script that is created by inery-launcher
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        ...
        killIneInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up ine walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.ineryAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.ineryAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.ineryAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.ineryAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.ineryAccount, buyMEM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.ineryAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.ineryAccount, buyMEM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountIneBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountIneBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killIneInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
